...
class AuthService {
  ...
  static async register(req) {
    ...
    if (!username || !email || !password || !role || !school_id) {
      throw new Error(
        "Missing required fields: username, email, password, role, or school_id",
      );
    }
    ...
    try {
      ...
      if (role === "school_admin") {
        ...
        if (existingAdmin.length > 0) {
          throw new Error(
            "School already has an administrator. Please contact them for access.",
          );
        }
      }
  ...
  /**
   * Super Admin Login — authenticates against the super_admin table (no school_id required)
   * @param {string} email - Super admin email
   * @param {string} password - Super admin password
   * @returns {Object} Super admin data and JWT token
   */
  static async superAdminLogin(email, password) {
    ...
    const [rows] = await db.query(
      `SELECT id, name, email, password_hash FROM super_admin WHERE email = ? LIMIT 1`,
      [email],
    );
    ...
    // Sign a JWT that carries role = 'super_admin'
    const token = jwt.sign(
      {
        user_id: admin.id,
        email: admin.email,
