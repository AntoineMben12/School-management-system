...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isSuperAdmin) {
        // Super admin has its own login endpoint — no school_id needed
        result = await authAPI.superAdminLogin({ email, password });
      } else {
      ...
      // Map DB role values → frontend routes
      const roleRoute = {
      ...
      navigate(roleRoute[result.user.role] || "/");
    } catch (err) {
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-8">
        ...
        <div className="w-full max-w-[480px] bg-white dark:bg-[#1e1e24] rounded-2xl shadow-apple p-8 md:p-12 border border-white/50 dark:border-white/5 relative flex flex-col justify-center">
          ...
          <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
            {/* School ID Field — hidden for Superadmin */}
            {!isSuperAdmin && (
              <div className="flex flex-col gap-2">
                <label
                  className="text-[#131118] dark:text-slate-200 text-sm font-medium leading-normal ml-1"
                  htmlFor="schoolId"
                >
                  School ID
                </label>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative group">
                <input
                  ...
                  id="email"
                  placeholder={
                    isSuperAdmin ? "superadmin@example.com" : "user@school.edu"
                  }
                  type="email"
                  value={email}
            ...
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center ml-1">
                <label
                  className="text-[#131118] dark:text-slate-200 text-sm font-medium leading-normal"
                  htmlFor="password"
                >
                  Password
                </label>
            ...
            <div className="flex items-center justify-between px-1">
              <label className="inline-flex items-center cursor-pointer">
                ...
                />
                <span className="ml-2 text-sm text-[#706189] dark:text-slate-400">
                  Remember me
                </span>
              </label>
              <a
